...
begin=clock();
	for(k=2;;k=k+2)
	{
		...
		for(i=0;i<N;i++)
		{
		T[i][0][k-1]=0.989154*T[i][1][k-2]+0.010864*10;
		}
		...
		for(i=1;i<N-1;i++)
		{
			...
			for(j=1;j<M-1;j++)
			{
				...
				D[j]=-(1+0.249984);
				a[j]=-0.5*0.249984;
				c[j]=-((0.5*0.249984)*(T[i+1][j][k-2]+T[i-1][j][k-2])+(1-0.249984)*T[i][j][k-2]);
		...
		for(j=1;j<M-1;j++)
		{
			...
			for(i=1;i<N-1;i++)
			{
				...
				D[i]=-(1+0.249984);
				a[i]=-0.5*0.249984;
				c[i]=-((0.5*0.249984)*(T[i][j+1][k-1]+T[i][j-1][k-1])+(1-0.249984)*T[i][j][k-1]);
